...
def plot_frb(cand, plot_range, profile, params):
    ...
    # highlight data included in the fit
    for item in params["fitrange"]:
        ax1.axvline(x=item, color="tab:orange", zorder=4)
    ...
    # highlight data included in the fit
    for item in params["fitrange"]:
        ax2.axvline(x=item, color="tab:orange", zorder=4)
    ...
    if not params["publish"]:
        for item, color, yval in zip(
            ...
            ax1.annotate(
                ...
                arrowprops=dict(arrowstyle="|-|", color=color, headwidth=10, lw=2),
                zorder=9,
            )
...
def plot_width_scaling(t_df, cand, fitresult, params, allcomp):
    ...
    # entire profile post-scattering values (observed)
    ax.errorbar(
        x=fact * df["cfreq"],
        y=df["weq"],
        yerr=df["err_weq"],
